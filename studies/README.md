...
### `kirkwood_field/` — ROADMAP §12, "The field reference stops being a Born ion"

| script | what it produced |
|---|---|
| `charge_proximity.py` | the `cells_out` ladder showing APBS's 20.96% at d/a = 0.9 is charge proximity, not the field |

The eight-case table beside it is not here because it is not a study: those
numbers are `analytic_field.max_relative_error` in the recordings themselves,
regenerated by `sashimi corpus build`.

### `lattice_phase/` — ROADMAP §12, "An observation this turned up, explained"

| script | what it produced |
|---|---|
| `debye_delphi_agreement.py` | why debye and DelPhi agree to five significant figures and APBS does not: two physics explanations refuted (`chgm spl0` moves APBS *away*, `bcfl mdh` moves it 0.006%), then the lattices — identical shape, spacing and origin for debye and DelPhi against APBS's re-centred box, and the two resolutions where their point-count rules diverge and the agreement goes with them |

Worth running before any claim that compares two grid backends in the near
field: it is the measurement that shows such a comparison can be reporting the
box rather than the solver.
